...
def get_data(filters):
    ...
    for work_order in work_orders:
        start_date = work_order.actual_start_date or work_order.planned_start_date
        work_order["age"] = 0

        if work_order.status != "Completed":
            work_order["age"] = date_diff(today(), start_date)

        # Calculate the difference in days between actual_start_date and actual_end_date
        work_order["days"] = 0
        if work_order.actual_start_date and work_order.actual_end_date:
            work_order["days"] = date_diff(work_order.actual_end_date, work_order.actual_start_date)

        if filters.get("age") <= work_order["age"]:
            work_order["indent"] = 0  # Main row
            result.append(work_order)

            # Fetch custom_item_attributes data for this Work Order
            custom_attributes = frappe.get_all(
                "Item Attributes",
                fields=["item","variant_attribute", "attribute_value"],
                filters={"parent": work_order["name"],"parenttype":"Work Order"},
                order_by="item",
            )

            # Add each attribute as a row with indent 1
            for attribute in custom_attributes:
                result.append({
               
                    "indent": 1,
                    "item_attribute": attribute["item"],
...
def get_columns(filters):
    ...
    columns.extend(
        [
            ...
            {"label": _("Variant Attribute"), "fieldname": "variant_attribute", "fieldtype": "Data", "width": 110},
            {"label": _("Attribute Value"), "fieldname": "Attribute_value", "fieldtype": "Data", "width": 110},
            {
    ...
    if filters.get("status") != "Not Started":
        columns.extend(
            [
                ...
                {"label": _("Days"), "fieldname": "days", "fieldtype": "Int", "width": 110},
