...
class CustomProductionPlan(Document):
    ...
    def get_open_sales_orders(self):
        ...
        open_so = get_sales_orders_with_extended_filters(self)
...
def get_sales_orders_with_extended_filters(self):
        bom = frappe.qb.DocType("BOM")
        pi = frappe.qb.DocType("Packed Item")
        ...
        # frappe.throw(str(open_so_subquery1))
        ...
        if self.get("custom_item_group"):
            item_codes = frappe.get_all(
                "Item", 
                filters={"item_group": self.custom_item_group}, 
                fields=["name"]
            )
            
            item_codes_list = [item["name"] for item in item_codes]
            
            if item_codes_list:
                open_so_query = open_so_query.where(so_item.item_code.isin(item_codes_list))





        if self.get("custom_warehouse"):
            open_so_query = open_so_query.where(so_item.warehouse == self.custom_warehouse)

        if self.get("custom_platform"):
            open_so_query = open_so_query.where(so.custom_platform == self.custom_platform)
        # Add condition for "only > 50%" checkbox
        if self.get("advance_paid_percentage"):
            open_so_query = open_so_query.where(
                ((so.advance_paid / so.rounded_total) * 100) >= self.get("advance_paid_percentage")
            )
